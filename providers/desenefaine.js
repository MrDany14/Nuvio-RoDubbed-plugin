...
// --- NEW FUNCTION: Extracts the direct .m3u8 from the iframe ---
function resolveIframeUrl(iframeUrl) {
    return fetchText(iframeUrl, { headers: { "Referer": MAIN_URL + "/" } }).then(function(html) {
        // Regex to find standard .m3u8 or .mp4 links in the player's HTML/JS
        ...
        return null; // Ignore errors, just return null
    });
}
...
function getStreams(id, type, season, episode) { 
  ...
  return fetchJson(tmdbUrl).then(function(data) { 
    ...
    return tryDirectUrl(roTitle).then(function(result) { 
      ...
      $$("source, video").each(function(_, el) { 
        ...
        if (src && (src.indexOf(".mp4") !== -1 || src.indexOf(".m3u8") !== -1)) { 
          ...
          streamPromises.push(Promise.resolve({ 
            ...
            url: src, 
            quality: "1080p", 
            isM3U8: src.includes(".m3u8"), // FIX: Tells Nuvio it's HLS
            headers: {
      ...
      // 2. FALLBACK: Resolve iframes to their direct stream URLs
      $$("iframe").each(function(_, el) { 
        ...
        // Push a promise that resolves the iframe
        var promise = resolveIframeUrl(src).then(function(directUrl) {
            if (directUrl) {
                log("RESOLVED IFRAME TO: " + directUrl);
                var iframeDomain = src.match(/^https?:\/\/([^/?#]+)/i)[0]; // e.g., https://player4me.com
                
                return { 
                    name: PROVIDER_NAME + " | Server " + serverCount++, 
                    title: "1080p | RO Dub", 
                    url: directUrl, // FIX: Direct .m3u8 link (No more loop)
                    quality: "1080p", 
                    isM3U8: directUrl.includes(".m3u8"), // FIX: Tells Nuvio it's HLS
                    headers: {
                        "User-Agent": FETCH_HEADERS["User-Agent"],
                        "Referer": iframeDomain + "/", // FIX: Bypasses CDN blocking
                        "Origin": iframeDomain
                    }, 
                    provider: "desenefaine" 
                };
            }
            return null; // Return null if it couldn't resolve
        });
        
        streamPromises.push(promise);
      }); 

      // Wait for all iframe resolutions to finish
      return Promise.all(streamPromises).then(function(resolvedStreams) {
          // Filter out the ones that failed (returned null)
          var finalStreams = resolvedStreams.filter(function(stream) { return stream !== null; });
          log("Extracted " + finalStreams.length + " valid streams."); 
          return finalStreams;
      });
