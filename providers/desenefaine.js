...
function server4TestStream() {
  return {
    ...
    isM3U8: false,
    behaviorHints: {
      bingeGroup: "filmedublate-server4-test"
    },
    provider: "filmedublate"
  };
}
...
function resolveProvider(url, wrapperUrl) {
  return Promise.resolve()
    ...
    .then(function(html) {
      ...
      if (mp4.length) {
        return resolveRedirectUrl(mp4[0], {
          headers: { Referer: url }
        })
...
function searchSite(query) {
  var queryWords = normalizeTitle(query)
    .split(" ")
...
function getStreams(id, type, season, episode) {
  ...
  return fetchJson(tmdbUrl(id, type))
    .then(function(data) {
      var isImdb = String(id).startsWith("tt");
      var roTitle = "";
      var enTitle = "";
        ...
        .then(function(result) {
          ...
          return fetchText(wrapperUrl, {
            ...
            while ((match = QUOTED_URL_RE.exec(wrapperHtml)) !== null) {
              ...
              if (!extractedUrl || !isLikelyPlayerUrl(extractedUrl)) {
                continue;
              }
