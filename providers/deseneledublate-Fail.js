...
function fetchWithTimeout(url, options, timeoutMs) {
  ...
  var settled = Promise.race([result, timeoutPromise]);
...
function resolvePlayer4meProvider(providerUrl, pageUrl) {
  ...
  return Promise.all([
    fetchText(infoUrl).then(decryptPlayer4meResponse).catch(function() { return null; }),
...
function readEpisodeFromSeriesPage(seriesResult, words, season, episode, expectedYear) {
  ...
  if (candidate) return readFilmPage(candidate, words, null, false);
  ...
  if (Object.keys(fallbackSeasons).length === 1 && seasonNumber === 1) {
    ...
    if (mappedEpisode) return readFilmPage(mappedEpisode, words, null, false);
    if (episodeNumber === 1 && fallbackCandidate) {
      return readFilmPage(fallbackCandidate, words, null, false);
    }
  }
              ...
              if (match.typeConflict || match.yearConflict || (!match.hasTitle && !hasEpisodes)) {
               ...
               if (/\/epi\//i.test(candidate.href)) {
                 ...
                 if (!episodePartMatches(candidateTokens, ["s", "season", "sezon", "sezonul"], season) ||
                  ...
                  return readFilmPage(candidate.href, words, null, false);
               }
